...
import jwer.backend.service.AuthenticationService
...
class SecurityConfig : WebSecurityConfigurerAdapter() {
    ...
    override fun configure(http: HttpSecurity) {
        http.authorizeRequests()
                ...
                ?.antMatchers(HttpMethod.GET, "/position/employee")?.hasRole(AccountRole.EMPLOYEE)
                ...
                ?.antMatchers("/schedule-generator")?.hasRole(AccountRole.EMPLOYER)
                ...
                ?.cors()
                ?.and()
                ?.addFilterAfter(SameSiteFilter(), BasicAuthenticationFilter::class.java)
    }
